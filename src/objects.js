...
SpriteMorph.prototype.setHighContrastBlockColor = function () {
  /*SpriteMorph.prototype.blockColor = {
    motion : new Color(128, 181, 255),
    looks : new Color (204, 179, 255),
    sound : new Color (225, 157, 225),
    pen : new Color (19, 236, 175),
    events : new Color (255, 217, 102),
    control : new Color (255, 190, 76),
    sensing : new Color(133, 196, 224),
    operators : new Color(126, 206, 126),
    variables : new Color(255, 165, 76),
    lists : new Color(255, 153, 102),
    other: new Color(191, 191, 191)
    };
    BlockMorph.prototype.zebraContrast = -20;*/
  SpriteMorph.prototype.isHighContrast = true;
};
...
SpriteMorph.prototype.bubbleFontSize = 12;
SpriteMorph.prototype.bubbleFontIsBold = false;
SpriteMorph.prototype.bubbleCorner = 12;
SpriteMorph.prototype.bubbleBorder = 2;
SpriteMorph.prototype.bubbleBorderColor = new Color (217, 217, 217);
  ...
  return {
    ...
    /*
        ...
        doGlideSprite: {
            ...
            type: 'command',
            category: 'motion',
            spec: 'glide %n secs to %dst',
            defaults: [1, ['random position']],
            animation: false,
            code: 'glide',
            src: `(
                (prim t doGlide span x y)
    ...
    reportAtan2: {
      type: "reporter",
      category: "operators",
      spec: "angle from center x: %n y: %n",
      code: "atan2",
      alias: "atan2",
    },
    reportVariadicMin: {
...
SpriteMorph.prototype.hasCustomizedPrimitives = function () {
  return Object.keys(this.blocks).some(
    (selector) =>
...
SpriteMorph.prototype.isBlocksAllTheWay = function () {
  ...
  return Object.keys(this.blocks).every((selector) => {
    ...
    return (
      record.definition instanceof CustomBlockDefinition ||
  ...
  spec = parts
    .map((word) => {
      if (word === "%br") {
        ...
        entry = CustomBlockDefinition.prototype.declarationFor(word);
  ...
  this.bootstrappedBlocks().forEach((def) => {
    ...
    if (prim && prim.selector === "doPrimitive" && prim.nextBlock()) {
      ...
      stage
        ...
        .reverse()
        .forEach(
          (block) => (block.selector = def.primitive || "evaluateCustomBlock")
        );
    }
  });
};
...
SpriteMorph.prototype.quasiPrimitives = function () {
  return this.bootstrappedBlocks().filter((def) => def.isQuasiPrimitive());
};
...
SpriteMorph.prototype.newPrimitivesSince = function (version) {
  ...
  if (version < 10) {
    selectors.push("reportNewCostumeSkewed");
  }
  if (version < 9.1) {
    selectors.push(
      "reportNewCostumeSkewed",
  ...
  if (version < 8.1) {
    selectors.push("reportPipe", "receiveUserEdit");
  }
  if (version < 8) {
    selectors.push(
      "getPosition",
  ...
  if (version < 10.2) {
    selectors.push("receiveSlotEvent", "doSetSlot");
  }
  if (version < 10.3) {
    selectors.push("receiveConditionEvent");
  }
  // 10.4 - 10.7: no new primitives
  if (version < 10.8) {
    selectors.push("reportColor", "reportColorAttribute", "reportNewColor");
  }
  if (version < 11) {
    selectors.push("reportPoll");
  }
...
SpriteMorph.prototype.fullCopy = function (forClone) {
  ...
  if (forClone) {
    ...
    this.customBlocks.forEach((def) => {
      ...
      c.allBlockInstances(def).forEach((block) => (block.definition = cb));
    });
...
SpriteMorph.prototype.fixLayout = function () {
  ...
  if (ide?.performerMode) {
    stageScale = ide.performerScale;
  }
  ...
  if (this.costume && !isLoadingCostume) {
    ...
    corners = pic
      ...
      .corners()
      .map((point) =>
        point.rotateBy(radians(facing - 90), this.costume.center())
      );
    origin = corners[0];
    corner = corners[0];
    ...
    costumeExtent = origin
      .corner(corner)
      .extent()
      .multiplyBy(this.scale * stageScale);
    ...
    if (this.rotationStyle === 1) {
      // rotate freely in all directions
      ...
      imageSide =
        Math.sqrt(Math.pow(pic.width(), 2) + Math.pow(pic.height(), 2)) *
        this.scale *
        stageScale;
    ...
    newX = Math.min(
      Math.max(this.normalExtent.x * this.scale * stageScale, 5),
      1000
    );
...
SpriteMorph.prototype.fullDrawOn = function (aContext, aRect) {
  ...
  this.isCachingImage =
    SpriteMorph.prototype.isCachingImage || this.graphicsChanged();
...
SpriteMorph.prototype.render = function (ctx) {
  ...
  if (this.costume && !isLoadingCostume) {
    ...
    if (isLoadingCostume) {
      // retry until costume is done loading
      cst = this.costume;
...
SpriteMorph.prototype.getImageData = function () {
  ...
  if (this.version !== this.imageData.version) {
    ...
    imageData = canvasContext.getImageData(0, 0, newExtent.x, newExtent.y).data;
    this.imageData = {
...
SpriteMorph.prototype.projectionSnap = function () {
  var stage = this.parentThatIsA(StageMorph),
    center = this.center().subtract(stage.position()).divideBy(stage.scale),
...
SpriteMorph.prototype.deleteVariableButton = function () {
  ...
  button = new PushButtonMorph(
    null,
    function () {
      var menu = new MenuMorph((vn) => myself.deleteVariable(vn), null, myself);
...
SpriteMorph.prototype.devModeText = function () {
  var txt = new TextMorph(localize("development mode \ndebugging primitives:"));
  txt.fontSize = 9;
  txt.setColor(this.paletteTextColor);
  return txt;
};
...
SpriteMorph.prototype.makeBlock = function () {
  ...
  dlg.prompt("Make a block", null, this.world());
};
...
SpriteMorph.prototype.palette = function (category) {
  if (
    !Object.hasOwn(this.paletteCache, category) ||
      ...
      .reduce((blocks, category) => {
        ...
        if (!showCategories && category !== "variables") {
          primitives = primitives.filter(
            (each) =>
              each !== "-" && each !== "=" && each && !each.hideWithCategory
          );
        }
...
SpriteMorph.prototype.allPaletteBlocks = function () {
  ...
  var blocks = SpriteMorph.prototype
    ...
    .reduce((blocks, category) => {
      ...
      return blocks.concat(primitives, customs);
    }, []);
...
SpriteMorph.prototype.isHidingBlock = function (aBlock) {
  ...
  if (aBlock.isCustomBlock) {
    return (
      aBlock.isGlobal ? aBlock.definition : this.getMethod(aBlock.semanticSpec)
    ).isHelper;
  }
...
SpriteMorph.prototype.changeBlockVisibility = function (aBlock, hideIt, quick) {
  ...
  if (aBlock.isCustomBlock) {
    ...
    this.variables.find(aBlock.blockSpec).vars[aBlock.blockSpec].isHidden =
      !!hideIt;
  } else {
    if (hideIt) {
...
SpriteMorph.prototype.emptyCategories = function () {
  ...
  var hasBlocks = (any) =>
    any instanceof BlockMorph && !this.isHidingBlock(any);
  if (this.categoriesCache === null) {
    this.categoriesCache = {};
    SpriteMorph.prototype
      .allCategories()
      .forEach(
        (category) =>
...
SpriteMorph.prototype.blocksMatching = function (
  ...
  function labelOf(aBlockSpec) {
    var words = BlockMorph.prototype.parseSpec(aBlockSpec),
      filtered = words.filter(
        (each) => each.indexOf("%") !== 0 || each.length === 1
      ),
      slots = words
        .filter((each) => each.length > 1 && each.indexOf("%") === 0)
  ...
  Object.keys(blocksDict).forEach((selector) => {
    if (
      !StageMorph.prototype.hiddenPrimitives[selector] &&
  ...
  return blocks.filter(
    (each) => !this.isHidingBlock(each) && !this.isDisablingBlock(each)
  );
};
...
SpriteMorph.prototype.searchBlocks = function (
  ...
  searchBar.setPosition(searchPane.contents.topLeft().add(new Point(10, 10)));
  searchBar.fixLayout();
  ...
  searchPane.accept = function () {
    ...
    if (scriptFocus) {
      ...
      myself.recordUserEdit(
        ...
        "block",
        "insert",
        selection instanceof BlockMorph ? selection.abstractBlockSpec() : ""
      );
    } else {
  ...
  searchPane.reactToKeystroke = function (evt) {
    var idx,
      code = evt ? evt.keyCode : 0;
    switch (code) {
      case 38: // up arrow
        if (!scriptFocus || !selection) {
          return;
        }
  ...
  searchPane.reactToInput = function (evt) {
    ...
    if (search !== oldSearch) {
      oldSearch = search;
      show(myself.blocksMatching(search, search.length < 2, types, varNames));
    }
  };
...
SpriteMorph.prototype.reporterize = function (expressionString) {
        ...
        case "|":
          if (!operator && !inputs[0].length) {
            ...
            if (!inputs[1].length) {
              ...
              return parseInfix(expression.slice(idx), ch, [
                operator,
                already,
                format(inputs[1]),
              ]);
  ...
  function blockFromAST(ast) {
    ...
    monads
      .concat(["true", "false"])
      .forEach((word) => (reverseDict[localize(word).toLowerCase()] = word));
    ...
    if (contains(monads, key)) {
      ...
      if (sel) {
        ...
        // two inputs, first is function name
      ...
      target = block;
      inps = block.inputs();
      if (inps[0] instanceof MultiArgMorph) {
        // infix sum or product
        target = inps[0];
        inps = target.inputs();
      }
    }
    for (i = 1; i < ast.length; i += 1) {
      if (ast[i] instanceof Array) {
        ...
        if (contains(["true", "false"], reverseDict[ast[i]] || ast[i])) {
          target.replaceInput(
            inps[i - off],
  ...
  if (expressionString.length > 100) {
    return null;
  }
  try {
...
SpriteMorph.prototype.renameVariable = function (
  ...
  function renameVariableInCustomBlock(definition) {
    definition.scripts.forEach((eachScript) => {
      if (eachScript instanceof BlockMorph) {
        // skip comments
        eachScript.refactorVariable(oldName, newName);
      }
    });
    if (definition.body) {
      definition.body.expression.refactorVariable(oldName, newName);
    }
  }

  if (contains(scope.names(), newName)) {
    ide.inform("Variable exists", "A variable with this name already exists.");
    return false;
  }
  ...
  if (everywhere) {
    targets = isGlobal
      ? ide.sprites
          .itemsArray()
          .concat([stage])
          .filter((sprite) => !contains(sprite.variables.names(), newName))
      : [this];
...
SpriteMorph.prototype.addCostume = function (costume) {
  ...
  this.recordUserEdit("costume", "add", costume.name);
};
...
SpriteMorph.prototype.doSwitchToCostumeNew = function (id, noShadow) {
  var w = 0,
    h = 0,
    stage;
  if (id instanceof List) {
...
SpriteMorph.prototype.doSwitchToCostumeNumber = function (id, noShadow) {
  var w = 0,
    h = 0,
    stage;
  if (+id != id) {
...
SpriteMorph.prototype.addSound = function (audio, name) {
  ...
  this.recordUserEdit("sounds", "add", sound.name);
  return sound;
};
...
SpriteMorph.prototype.doPlaySound = function (name) {
  var stage = this.parentThatIsA(StageMorph),
    sound =
      name instanceof Sound
        ? name
        : typeof name === "number"
        ? this.sounds.at(name)
        : detect(this.sounds.asArray(), (s) =>
            snapEquals(s.name, name.toString())
          ),
...
SpriteMorph.prototype.setPan = function (num, noShadow) {
  ...
  panner.pan.setValueAtTime(this.pan / 100, this.audioContext().currentTime);
  if (this instanceof StageMorph) {
    return;
  }
...
SpriteMorph.prototype.userMenu = function () {
  ...
  if (this.anchor) {
    ...
    anchors = this.parent.children.filter(
      (morph) => morph instanceof SpriteMorph && !contains(allParts, morph)
    );
    if (anchors.length) {
...
SpriteMorph.prototype.anchorsMenu = function (targets) {
  ...
  targets.forEach((sprite) =>
    menu.addItem([sprite.thumbnail(new Point(24, 24)), sprite.name], sprite)
  );
  return menu;
};
...
SpriteMorph.prototype.toXMLString = function () {
  ...
  all.forEach((sprite) => {
    ...
    sprite.scripts.children
      .filter((morph) => morph instanceof BlockMorph)
      .forEach((script) => collectAll(script.dependencies(true), dependencies));
    ...
    sprite.customBlocks.forEach((def) => {
      ...
      collectAll(
        def.collectDependencies([], [], sprite).filter((each) => each.isGlobal),
        dependencies
      );
    });
  });
...
SpriteMorph.prototype.clonify = function (stage, immediately) {
  ...
  stage.cloneCount += 1;
  this.cloneOriginName = this.isTemporary ? this.cloneOriginName : this.name;
...
SpriteMorph.prototype.initClone = function (hats) {
  ...
  if (stage) {
    hats.forEach((block) =>
      stage.threads.startProcess(block, this, stage.isThreadSafe)
    );
    this.endWarp();
  }
};
...
SpriteMorph.prototype.perpetuateAndEdit = function () {
  ...
  if (ide) {
    this.perpetuate();
    ide.selectSprite(this);
    this.recordUserEdit("corral", "permanent clone", this.name);
  }
};
...
SpriteMorph.prototype.changeColorDimension = function (idx, delta) {
  this.setColorDimension(idx, this.getColorDimension(idx) + (+delta || 0));
};
...
SpriteMorph.prototype.neighbors = function (aStage) {
  ...
  return new List(
    stage.children.filter((each) => {
      var eachPerimeter, distance;
      if (each instanceof SpriteMorph && each.isVisible && each !== this) {
        eachPerimeter = each.perimeter(stage);
        distance = myPerimeter.center.distanceTo(eachPerimeter.center);
        return distance - myPerimeter.radius - eachPerimeter.radius < 0;
      }
      return false;
    })
  );
...
SpriteMorph.prototype.perimeter = function (aStage) {
  ...
  if (this.costume) {
    radius =
      Math.max(this.costume.width(), this.costume.height()) *
      this.scale *
      stageScale;
  } else {
...
SpriteMorph.prototype.doStamp = function () {
  ...
  ctx.drawImage(img, this.left() - stage.left(), this.top() - stage.top());
  ctx.restore();
  this.changed();
...
SpriteMorph.prototype.write = function (text, size) {
  ...
  if (typeof text !== "string" && typeof text !== "number") {
    throw new Error(
      localize("can only write text or numbers, not a") + " " + typeof text
    );
  }
...
SpriteMorph.prototype.blitOn = function (target, mask = "source-atop") {
  ...
  if (target instanceof SpriteMorph) {
    if (this instanceof SpriteMorph) {
      ...
      spriteCenter = new Point(target.costume.width(), target.costume.height())
      ...
      centerAngleRadians = Math.atan2(centerDelta.y, centerDelta.x);
      center = new Point(target.costume.width(), target.costume.height())
    ...
    pos = center.subtract(
      new Point(sourceCostume.width(), sourceCostume.height()).multiplyBy(0.5)
    );
  }
...
SpriteMorph.prototype.graphicsChanged = function () {
  return Object.keys(this.graphicsValues).some(
    (any) => this.graphicsValues[any] < 0 || this.graphicsValues[any] > 0
  );
};
...
SpriteMorph.prototype.talkBubble = function () {
  return detect(this.children, (morph) => morph instanceof SpeechBubbleMorph);
};
...
SpriteMorph.prototype.prepareToBeGrabbed = function (hand) {
  ...
  if (
    !this.bounds.containsPoint(hand.position()) &&
...
SpriteMorph.prototype.moveBy = function (delta, justMe) {
  ...
  var start =
    this.isDown && !justMe && this.parent ? this.rotationCenter() : null;
...
SpriteMorph.prototype.setHeading = function (degrees, noShadow) {
  ...
  if (this.rotationStyle) {
    // optimization, only redraw if rotatable
    this.changed();
    ...
    angle =
      Math.abs(deltaX) < 0.001
        ? deltaY < 0
          ...
          : 270
        : Math.round(
            (deltaX >= 0 ? 0 : 180) - Math.atan(deltaY / deltaX) * 57.2957795131
          );
  return angle + 90;
};
...
SpriteMorph.prototype.bounceOffEdge = function () {
  ...
  this.setPosition(
    this.position().add(fb.amountToTranslateWithin(stage.bounds))
  );
  this.positionTalkBubble();
};
...
SpriteMorph.prototype.setRotationCenter = function (absoluteCoordinate) {
  ...
  this.shadowAttribute("costumes");
  delta = absoluteCoordinate
    .subtract(new Point(this.xPosition(), this.yPosition()))
    .divideBy(this.scale)
    .rotateBy(radians(90 - this.heading));
...
SpriteMorph.prototype.setPivot = function (worldCoordinate) {
  ...
  if (stage) {
    ...
    this.recordUserEdit("sprite", "pivot", this.xPosition(), this.yPosition());
  }
};
  ...
  this.allScripts().forEach((script) => {
    script.allChildren().forEach((morph) => {
      ...
      if (
        ...
        morph.choices &&
        contains(["messagesMenu", "messagesReceivedMenu"], morph.choices)
      ) {
        txt = morph.evaluate();
        if (isString(txt) && txt !== "") {
  ...
  return this.scripts.children.filter((morph) => {
    if (morph.selector) {
      if (morph.selector === "receiveInteraction") {
        var choice = morph.inputs()[0].evaluate();
        return (choice instanceof Array ? choice[0] : choice) === interaction;
      }
    }
    return false;
  });
...
SpriteMorph.prototype.hasGenericHatBlocks = function () {
  ...
  return this.scripts.children.some(
    (morph) =>
      morph instanceof CustomHatBlockMorph || generics.includes(morph.selector)
  );
};

SpriteMorph.prototype.allGenericHatBlocks = function () {
  var generics = ["receiveCondition", "receiveConditionEvent"];
  return this.scripts.children.filter(
    (morph) =>
      morph instanceof CustomHatBlockMorph || generics.includes(morph.selector)
  );
};
...
SpriteMorph.prototype.deleteAllBlockInstances = function (definition) {
  var stage,
    blocks = definition.isGlobal
      ? this.allBlockInstances(definition)
...
SpriteMorph.prototype.allBlockInstances = function (definition) {
  ...
  if (definition.isGlobal) {
    ...
    stage.globalBlocks
      .concat(SpriteMorph.prototype.bootstrappedBlocks())
      .forEach((def) => {
    ...
    return blocks
      .concat(inDefinitions)
      .concat(stage.allBlockInstancesInData(definition));
  }
...
SpriteMorph.prototype.allIndependentInvocationsOf = function (aSpec) {
  ...
  this.instances.forEach((sprite) => sprite.addAllInvocationsOf(aSpec, blocks));
  return blocks;
};
...
SpriteMorph.prototype.allInvocationsOf = function (aSpec) {
  ...
  inScripts = this.scripts
    .allChildren()
    .filter((c) => c.isCustomBlock && !c.isGlobal && c.blockSpec === aSpec);

  inDefinitions = [];
  this.customBlocks.forEach((def) => {
    def.scripts.forEach((eachScript) =>
      eachScript.allChildren().forEach((c) => {
        if (c.isCustomBlock && !c.isGlobal && c.blockSpec === aSpec) {
          inDefinitions.push(c);
        }
      })
    );
...
SpriteMorph.prototype.addAllInvocationsOf = function (aSpec, anArray) {
  if (!this.getLocalMethod(aSpec)) {
    this.allInvocationsOf(aSpec).forEach((block) => anArray.push(block));
...
SpriteMorph.prototype.allLocalBlockInstances = function (definition) {
  ...
  inScripts = this.scripts
    .allChildren()
    .filter((c) => c.isCustomBlock && c.definition === definition);
...
SpriteMorph.prototype.usesBlockInstance = function (
  ...
  if (definition.isGlobal && !skipGlobals) {
    ...
    this.parentThatIsA(StageMorph)
      .globalBlocks.concat(this.customizedPrimitives())
      .forEach((def) => {
        if (forRemoval && definition === def) {
          return;
        }
        if (skipBlocks && contains(skipBlocks, def)) {
          return;
        }
        if (def.body) {
          def.body.expression.allChildren().forEach((c) => {
...
SpriteMorph.prototype.replaceDoubleDefinitionsFor = function (definition) {
  ...
  if (definition.isGlobal) {
    ...
    this.allDependentInvocationsOf(definition.blockSpec())
      .reverse()
      .forEach((block) => block.refresh(definition));
  }
  ide = this.parentThatIsA(IDE_Morph);
  if (ide) {
    ide.flushPaletteCache();
...
SpriteMorph.prototype.setExemplar = function (another, enableError) {
  ...
  if (
    another instanceof SpriteMorph &&
...
SpriteMorph.prototype.allSpecimens = function () {
  ...
  this.instances.forEach((child) => all.push.apply(all, child.allSpecimens()));
  return all;
};
...
SpriteMorph.prototype.shadowAllAttributes = function () {
  this.attributes.forEach((att) => this.shadowAttribute(att));
};

SpriteMorph.prototype.shadowAttribute = function (aName) {
  var ide, wardrobe, jukebox, pos;
...
SpriteMorph.prototype.toggleInheritedVariable = function (vName) {
  if (contains(this.inheritedVariableNames(true), vName)) {
    // is shadowed
    this.deleteVariable(vName);
  } else if (contains(this.inheritedVariableNames(), vName)) {
    // inherited
...
SpriteMorph.prototype.inheritedVariableNames = function (shadowedOnly) {
  ...
  while (current.owner instanceof SpriteMorph) {
    names.push.apply(names, current.names().filter(test));
    current = current.parentFrame;
  }
  return names;
};
...
SpriteMorph.prototype.deletableVariableNames = function () {
  ...
  return locals.concat(
    this.globalVariables()
      .names()
      .filter((each) => !contains(locals, each) && !contains(inherited, each))
  );
};
...
SpriteMorph.prototype.ownBlocks = function () {
  var dict = {};
  this.customBlocks.forEach((def) => (dict[def.blockSpec()] = def));
  return dict;
};
...
SpriteMorph.prototype.allBlocks = function (valuesOnly) {
  ...
  this.allExemplars()
    .reverse()
    .forEach((sprite) =>
      sprite.customBlocks.forEach((def) => (dict[def.blockSpec()] = def))
    );
  if (valuesOnly) {
...
SpriteMorph.prototype.shadowAllMethods = function () {
  var ide;
  this.inheritedMethods().forEach((dup) => this.customBlocks.push(dup));
  if (!this.isTemporary) {
    ide = this.parentThatIsA(IDE_Morph);
    if (ide) {
      ide.flushPaletteCache();
...
SpriteMorph.prototype.thumbnail = function (extentPoint, recycleMe, noCorpse) {
  ...
  var src = this.getImage(), // at this time sprites aren't composite morphs
    w = this.width(),
    h = this.height(),
    scale = Math.min(extentPoint.x / w, extentPoint.y / h),
...
SpriteMorph.prototype.fullThumbnail = function (extentPoint, recycleMe) {
  ...
  ctx.restore();
  if (this.anchor) {
    ctx.drawImage(this.anchor.thumbnail(ext), 0, 0);
  }
  for (i = 0; i < 3; i += 1) {
    if (this.parts[i]) {
      ctx.drawImage(
...
SpriteMorph.prototype.colorSwatch = function (color, size) {
  ...
  swatch.render = function (ctx) {
    ...
    if (clr.a < 1) {
      ...
      ctx.fillRect(4, 4, w * 0.5 - 4, w * 0.5 - 4);
      ctx.fillRect(w * 0.5, w * 0.5, w * 0.5 - 4, w * 0.5 - 4);
    }
...
SpriteMorph.prototype.attachPart = function (aSprite) {
  ...
  this.version = v;
  aSprite.anchor = this;
  this.allParts().forEach((part) => (part.nestingScale = part.scale));
  aSprite.version = v;
};
...
SpriteMorph.prototype.allParts = function () {
  // includes myself
  var result = [this];
  this.parts.forEach((part) => (result = result.concat(part.allParts())));
  return result;
};
...
SpriteMorph.prototype.recordLayers = function () {
  ...
  this.layers.sort((x, y) =>
    stage.children.indexOf(x) < stage.children.indexOf(y) ? -1 : 1
  );
};
...
SpriteMorph.prototype.flash = function () {
  ...
  world.animations.push(
    new Animation(nop, nop, 0, 800, nop, () => this.removeHighlight())
  );
};
...
SpriteMorph.prototype.highlight = function (color, border) {
  ...
  ctx.drawImage(this.highlightImage(WHITE, 4), border - 4, border - 4);
  ctx.drawImage(
    this.highlightImage(new Color(50, 50, 50), 2),
    border - 2,
    border - 2
  );
  ctx.drawImage(this.highlightImage(WHITE, 1), border - 1, border - 1);
...
SpriteMorph.prototype.getHighlight = function () {
  var highlights = this.children
    .slice(0)
    .reverse()
    .filter((child) => child instanceof SpriteHighlightMorph);
...
SpriteMorph.prototype.newCostumeName = function (name, ignoredCostume) {
  var ix = name.indexOf("("),
    ...
    all = this.costumes
      .asArray()
      .filter((each) => each !== ignoredCostume)
      .map((each) => each.name),
...
SpriteMorph.prototype.recordUserEdit = function (...details) {
  ...
  if (ide) {
    ide.recordUnsavedChanges(
      this.name,
      Array.from(details).concat(
        details[0] === "scripts" ? [this.scriptsOnlyXML()] : []
      )
    );
  }
};
...
SpriteMorph.prototype.scriptsOnlyXML = function () {
  var serializer =
    this.parentThatIsA(IDE_Morph)?.serializer || new SnapSerializer();
  return (
    "<scriptsonly" +
    ' app="' +
    serializer.app +
...
SpriteMorph.prototype.synchScriptsFrom = function (xml) {
  ...
  try {
    ...
    serializer.loadScripts(this, this.scripts, serializer.parse(xml, true));
    this.scripts.changed();
    this.recordUserEdit("sprite", "synch", "scripts", xml);
  } catch (err) {
...
StageMorph.prototype.init = function (globals) {
  ...
  this.isCachingImage = true;
  this.cachedColorDimensions =
    this.color[SpriteMorph.prototype.penColorModel]();
...
StageMorph.prototype.startVideo = function () {
  ...
  if (this.projectionSource) {
    // video capture has already been started
    return;
  }
...
StageMorph.prototype.stopVideo = function () {
  if (this.projectionSource && this.projectionSource.stream) {
    this.projectionSource.stream.getTracks().forEach((track) => track.stop());
  }
  this.videoMotion = null;
};
...
StageMorph.prototype.processKeyEvent = function (event, action) {
  ...
  switch (event.keyCode) {
    ...
    default:
      keyName =
        event.key || String.fromCharCode(event.keyCode || event.charCode);
      if (event.ctrlKey || event.metaKey) {
        keyName =
...
StageMorph.prototype.fireKeyEvent = function (key) {
  ...
  if (evt === "ctrl shift z" || evt === "ctrl y") {
    if (!ide.isAppMode) {
      ide.currentSprite.scripts.redrop();
    }
    return;
  }
  if (evt === "ctrl n") {
  ...
  if (evt === "ctrl shift s") {
    if (!ide.isAppMode) {
      return ide.saveProjectsBrowser();
    }
    return;
  }
      ...
      morph.allHatBlocksFor(message).forEach((block) => {
        ...
        if (block.selector === "receiveMessage") {
          ...
          if (varName) {
            ...
            if (choice instanceof Array && choice[0].indexOf("any") === 0) {
              varFrame.addVar(
                varName,
                data !== "" ? new List([message, data]) : message
              );
            } else {
          ...
          procs.push(
            this.threads.startProcess(block, morph, this.isThreadSafe)
          );
        }
      });
    }
    ...
    if (isSnapObject(morph)) {
      morph.allHatBlocksForUserEdit(spriteName).forEach((block) => {
        ...
        if (varName) {
          ...
          varFrame.addVar(
            varName,
            new List([spriteName].concat(details).concat([timestamp]))
          );
        }
        procs.push(
          this.threads.startProcess(
...
StageMorph.prototype.editScripts = function () {
  var ide = this.parentThatIsA(IDE_Morph),
    scripts,
    sorted;
  if (ide.isAppMode || !ScriptsMorph.prototype.enableKeyboard) {
    return;
  }
  scripts = this.parentThatIsA(IDE_Morph).currentSprite.scripts.selectForEdit(); // shadow on edit, if inherited
...
StageMorph.prototype.hiddenGlobalBlocks = function () {
  ...
  return new List([
    new List(
      Object.keys(this.hiddenPrimitives).filter(
        (selector) => this.hiddenPrimitives[selector]
      )
    ),
    new List(
      this.globalBlocks
        .filter((def) => def.isHelper)
        .map((def) => def.abstractBlockSpec())
    ),
...
StageMorph.prototype.restoreHiddenGlobalBlocks = function (hiddenList) {
  ...
  this.globalBlocks.forEach(
    (def) => (def.isHelper = hiddenList.at(2).contains(def.abstractBlockSpec))
  );
  variables
    .names(true)
    .forEach(
  ...
  ide.refreshPalette();
  ide.categories.refreshEmpty();
  this.recordUserEdit("palette", "restore microworld");
};
...
StageMorph.prototype.addSwitchToScriptsButton = function () {
  ...
  switchToScriptsButton.hint = "toggle focus between stage\nand scripting area";
...
StageMorph.prototype.fancyThumbnail = function (
  ...
  this.children.forEach((morph) => {
    if (
      (isSnapObject(morph) || !noWatchers) &&
      morph.isVisible &&
      morph !== excludedSprite
    ) {
      fb = morph.fullBounds();
...
StageMorph.prototype.setColorRGBA = SpriteMorph.prototype.setColorRGBA;

StageMorph.prototype.changeColorRGBA = SpriteMorph.prototype.changeColorRGBA;
...
StageMorph.prototype.getPenAttribute = SpriteMorph.prototype.getPenAttribute;
...
StageMorph.prototype.allPaletteBlocks = SpriteMorph.prototype.allPaletteBlocks;
...
StageMorph.prototype.emptyCategories = SpriteMorph.prototype.emptyCategories;
...
StageMorph.prototype.getEffect = SpriteMorph.prototype.getEffect;
...
StageMorph.prototype.doPlaySound = SpriteMorph.prototype.doPlaySound;
...
StageMorph.prototype.newSoundName = SpriteMorph.prototype.newSoundName;

// StageMorph volume

StageMorph.prototype.setVolume = SpriteMorph.prototype.setVolume;

StageMorph.prototype.changeVolume = SpriteMorph.prototype.changeVolume;

StageMorph.prototype.getVolume = SpriteMorph.prototype.getVolume;

StageMorph.prototype.getGainNode = SpriteMorph.prototype.getGainNode;

StageMorph.prototype.audioContext = SpriteMorph.prototype.audioContext;

// StageMorph stereo panning

StageMorph.prototype.setPan = SpriteMorph.prototype.setPan;

StageMorph.prototype.changePan = SpriteMorph.prototype.changePan;

StageMorph.prototype.getPan = SpriteMorph.prototype.getPan;

StageMorph.prototype.getPannerNode = SpriteMorph.prototype.getPannerNode;

// StageMorph frequency player

StageMorph.prototype.playFreq = SpriteMorph.prototype.playFreq;

StageMorph.prototype.stopFreq = SpriteMorph.prototype.stopFreq;

// StageMorph non-variable watchers

StageMorph.prototype.toggleWatcher = SpriteMorph.prototype.toggleWatcher;

StageMorph.prototype.showingWatcher = SpriteMorph.prototype.showingWatcher;

StageMorph.prototype.watcherFor = SpriteMorph.prototype.watcherFor;
...
StageMorph.prototype.snapPoint = SpriteMorph.prototype.snapPoint;

StageMorph.prototype.worldPoint = SpriteMorph.prototype.worldPoint;
...
StageMorph.prototype.allMessageNames = SpriteMorph.prototype.allMessageNames;

StageMorph.prototype.allSendersOf = SpriteMorph.prototype.allSendersOf;
...
StageMorph.prototype.allScripts = SpriteMorph.prototype.allScripts;
...
StageMorph.prototype.mouseScroll = SpriteMorph.prototype.mouseScroll;
...
StageMorph.prototype.allContextsUsing = function (definition) {
  ...
  function scanContext(context) {
    ...
    if (
      ...
      context.expression
        .allChildren()
        .some((c) => c.isCustomBlock && c.definition === definition)
    ) {
...
StageMorph.prototype.allBlockInvocationsInData = function (oldSpec, receiver) {
  ...
  this.allContextsInvoking(oldSpec, receiver).forEach((context) => {
    if (context.expression instanceof BlockMorph) {
      context.expression.allChildren().forEach((c) => {
        if (c.isCustomBlock && !c.isGlobal && c.blockSpec === oldSpec) {
          blocks.push(c);
        }
      });
    }
...
StageMorph.prototype.allContextsInvoking = function (oldSpec, receiver) {
  ...
  function scanContext(context) {
    ...
    if (
      (context.receiver === receiver || context.receiver === null) &&
      context.expression instanceof BlockMorph &&
      context.expression
        .allChildren()
        .some((c) => c.isCustomBlock && !c.isGlobal && c.blockSpec === oldSpec)
    ) {
      contexts.push(context);
    }
  }
...
StageMorph.prototype.allInvocationsOf = SpriteMorph.prototype.allInvocationsOf;
...
StageMorph.prototype.customizeBlocks = SpriteMorph.prototype.customizeBlocks;

StageMorph.prototype.restorePrimitives =
  SpriteMorph.prototype.restorePrimitives;

StageMorph.prototype.restorePrimitive = SpriteMorph.prototype.restorePrimitive;
...
StageMorph.prototype.globalVariables = SpriteMorph.prototype.globalVariables;
...
StageMorph.prototype.getMethod = SpriteMorph.prototype.getMethod;

StageMorph.prototype.getLocalMethod = SpriteMorph.prototype.getLocalMethod;

StageMorph.prototype.ownBlocks = SpriteMorph.prototype.ownBlocks;
...
StageMorph.prototype.recordUserEdit = SpriteMorph.prototype.recordUserEdit;

StageMorph.prototype.scriptsOnlyXML = SpriteMorph.prototype.scriptsOnlyXML;

StageMorph.prototype.synchScriptsFrom = SpriteMorph.prototype.synchScriptsFrom;

// StageMorph cloning a generic sprite

StageMorph.prototype.newTurtleSprite = SpriteMorph.prototype.newTurtleSprite;
...
StageMorph.prototype.globalBlocksSending = function (message, receiverName) {
  // "transitive hull"
  var all = this.globalBlocks.filter((def) =>
    def.isSending(message, receiverName)
  );
  this.globalBlocks.forEach((def) => {
...
StageMorph.prototype.toXMLString = function () {
  ...
  this.customBlocks.forEach((def) => {
    collect(def.category, categories);
    collectAll(
      def.collectDependencies([], [], this).filter((each) => each.isGlobal),
      dependencies
    );
  });
...
SpriteBubbleMorph.prototype.init = function (
  ...
  this.bubbleFontColor = new Color (87, 94, 117);
...
SpriteBubbleMorph.prototype.dataAsMorph = function (data) {
  var contents,
    ...
    maxHeight =
      (this.stage?.dimensions?.y || 360) * this.scale -
  ...
  if (data instanceof Morph) {
    ...
    contents.userMenu = function () {
      var menu = new MenuMorph(this),
        ide =
          this.parentThatIsA(IDE_Morph) || this.world().childThatIsA(IDE_Morph);

      if (ide.isAppMode) {
        return;
      }
      menu.addItem("export", () =>
        ide.saveFileAs(data, "text/plain;charset=utf-8", localize("data"))
      );
      menu.addItem("copy", () => writeClipboardText(data, ide));
      return menu;
    };
    ...
    contents.userMenu = function () {
      ...
      menu.addItem("export", () => {
        if (data instanceof SVG_Costume) {
          ...
          ide.saveFileAs(data.contents.src, "text/svg", data.name);
        } else {
          // rasterized Costume
    ...
    contents.userMenu = function () {
      ...
      menu.addItem("export", () => ide.saveAudioAs(data.audio, data.name));
      return menu;
    };
    ...
    if (data.isTable()) {
      ...
      if (this.stage) {
        contents.expand(
          this.stage
            .extent()
            .translateBy(-2 * (this.edge + this.border + this.padding))
        );
      }
    } else {
  ...
  if (contents instanceof TextMorph) {
    ...
    scaledImg
      .getContext("2d")
      .drawImage(contents.getImage(), 0, 0, scaledImg.width, scaledImg.height);
...
SpriteBubbleMorph.prototype.fixLayout = function () {
  ...
  this.contentsMorph.setPosition(
    this.position().add(
      new Point(this.padding || this.edge, this.border + this.padding + 1)
    )
  );
};
...
StageBubbleMorph.prototype.adjustDimensions = function () {
  ...
  this.bounds.setHeight(
    this.contentsMorph.height() + this.edge + this.border * 2
  );
};
...
function Costume(canvas, name, rotationCenter, noFit, maxExtent) {
  this.contents = canvas
    ? normalizeCanvas(canvas, true)
    : newCanvas(null, true);
  if (!noFit) {
    this.shrinkToFit(maxExtent || this.maxExtent());
  }
  this.name = name || null;
...
*/
  ...
  flipped = new Costume(
    canvas,
    this.name,
    new Point(this.width() - this.rotationCenter.x, this.rotationCenter.y),
    true // no shrink-wrap
  );
  return flipped;
};
...
Costume.prototype.stretched = function (w, h) {
  ...
  stretched = new Costume(canvas, this.name, center, true);
  return stretched;
};
...
Costume.prototype.edit = function (aWorld, anIDE, isnew, oncancel, onsubmit) {
  ...
  editor.openIn(
    aWorld,
    isnew ? newCanvas(anIDE.stage.dimensions, true) : this.contents,
...
Costume.prototype.thumbnail = function (extentPoint, recycleMe, noPadding) {
  ...
  var src = this.contents,
    ...
    scale = Math.min(extentPoint.x / w, extentPoint.y / h),
    xOffset = noPadding ? 0 : Math.floor((extentPoint.x - w * scale) / 2),
    yOffset = noPadding ? 0 : Math.floor((extentPoint.y - h * scale) / 2),
    trg,
    ctx;

  trg = newCanvas(
    noPadding
      ? new Point(this.width() * scale, this.height() * scale)
      : extentPoint,
    true, // non-retina
    recycleMe
  );
...
Costume.prototype.pixels = function () {
  ...
  src = this.contents
    .getContext("2d")
    .getImageData(0, 0, this.contents.width, this.contents.height);
  for (i = 0; i < src.data.length; i += 4) {
    pixels.push(
      new List([src.data[i], src.data[i + 1], src.data[i + 2], src.data[i + 3]])
    );
  }
  return new List(pixels);
};
...
SVG_Costume.prototype.rasterized = function () {
  ...
  ctx.drawImage(this.contents, 0, 0);
  rasterized = new Costume(canvas, this.name, this.rotationCenter.copy());
  return rasterized;
};
...
CostumeEditorMorph.prototype.render = function (ctx) {
  ...
  ctx.fillStyle = "white";
  ctx.beginPath();
  ctx.arc(rp.x, rp.y, 20, radians(0), radians(360), false);
  ctx.closePath();
  ctx.fill();
  ctx.stroke();

  ctx.beginPath();
  ctx.arc(rp.x, rp.y, 10, radians(0), radians(360), false);
  ctx.stroke();
...
CostumeEditorMorph.prototype.mouseDownLeft = function (pos) {
  this.rotationCenter = pos.subtract(this.position().add(this.margin));
  this.rerender();
};
...
Note.prototype.setInstrument = function (type) {
  ...
  if (this.oscillator) {
    this.oscillator.type = ["sine", "square", "sawtooth", "triangle"][
      (type || 1) - 1
    ];
  }
};
...
Note.prototype.stop = function (immediately) {
  ...
  if (this.oscillator) {
    this.oscillator.stop(
      fade ? this.audioContext.currentTime + this.fadeTime : 0
    );
    this.oscillator = null;
  }
  this.ended = true;
...
Microphone.prototype.start = function () {
  ...
  navigator.mediaDevices
    ...
    .then((stream) => this.setupNodes(stream))
    .catch(nop);
};
...
Microphone.prototype.stepAudio = function (event) {
  var channels, i;
  if (this.isAutoStop && Date.now() - this.lastTime > 5000 && !this.modifier) {
    this.stop();
    return;
  }
  ...
  if (this.pitch > 0) {
    this.note =
      Math.round(12 * (Math.log(this.pitch / 440) / Math.log(2))) + 69;
  }
...
CellMorph.prototype.createContents = function () {
  ...
  if (!isSameList && !isSameTable) {
    if (this.contents instanceof Morph) {
      ...
      txt =
        this.contents.length > 500
          ? this.contents.slice(0, 500) + "..."
          : this.contents;
      ...
      if (this.contents.isTable()) {
        this.contentsMorph = new TableFrameMorph(
          new TableMorph(this.contents, 10)
        );
...
CellMorph.prototype.reactToEdit = function (textMorph) {
  ...
  if (!isNil(this.idx)) {
    listWatcher = this.parentThatIsA(ListWatcherMorph);
    if (listWatcher) {
      listWatcher.list.put(textMorph.text, this.idx + listWatcher.start - 1);
    }
  }
};
...
CellMorph.prototype.mouseDoubleClick = function (pos) {
  if (List.prototype.enableTables && this.currentValue instanceof List) {
  ...
  if (this.target && this.getter) {
    ...
    if (this.target instanceof VariableFrame) {
      newValue = this.target.vars[this.getter]
        ? this.target.vars[this.getter].value
        : undefined;
      if (newValue === undefined && this.target.owner) {
        ...
        if (contains(sprite.inheritedVariableNames(), this.getter)) {
          ...
          this.cellMorph.setColor(
            SpriteMorph.prototype.blockColor.variables.lighter(35)
          );
        } else {
          this.destroy();
          return;
...
WatcherMorph.prototype.fixLayout = function () {
  ...
  if (isList) {
    ...
    this.cellMorph.setPosition(
      this.labelMorph.topRight().add(new Point(fontSize / 3, 0))
    );
...
WatcherMorph.prototype.userMenu = function () {
  ...
  if (this.target instanceof VariableFrame) {
    ...
    menu.addLine();
    menu.addItem("import...", "importData");
    menu.addItem(
      "raw data...",
    ...
    if (shiftClicked) {
      if (this.currentValue instanceof List && this.currentValue.canBeCSV()) {
        menu.addItem(
          "export as CSV...",
          () =>
            ide.saveFileAs(
      ...
      if (this.currentValue instanceof List && this.currentValue.canBeJSON()) {
        menu.addItem(
          "export as JSON...",
          () =>
            ide.saveFileAs(
    ...
    if (isString(this.currentValue) || !isNaN(+this.currentValue)) {
      ...
      menu.addItem("export...", () => {
        if (this.currentValue instanceof SVG_Costume) {
          ...
          // rasterized Costume
          ide.saveCanvasAs(this.currentValue.contents, this.currentValue.name);
        }
      });
    } else if (this.currentValue instanceof Sound) {
      menu.addItem("export...", () =>
        ide.saveAudioAs(this.currentValue.audio, this.currentValue.name)
      );
    } else if (
  ...
  function userImport() {
    ...
    function readText(aFile) {
      ...
      if (raw || isTextFile(aFile)) {
        ...
        txtOnlyMsg(aFile.type, () => frd.readAsText(aFile));
      }
    }
...
WatcherMorph.prototype.userSetSliderMin = function () {
  new DialogBoxMorph(this, this.setSliderMin, this).prompt(
...
WatcherMorph.prototype.userSetSliderMax = function () {
  new DialogBoxMorph(this, this.setSliderMax, this).prompt(
...
WatcherMorph.prototype.render = function (ctx) {
  var gradient;
  var isFlat =
    MorphicPreferences.isFlat || (this.edge === 0 && this.border === 0);
  if (isFlat) {
    gradient = this.color.toString();
  } else {
...
StagePrompterMorph.prototype.init = function (question) {
  ...
  this.button = new PushButtonMorph(null, () => this.accept(), "\u2713");
...
StagePrompterMorph.prototype.fixLayout = function () {
  ...
  if (this.label) {
    this.label.setPosition(
      new Point(this.left() + this.edge, this.top() + this.edge)
    );
    y = this.label.bottom() - this.top();
  }
  this.inputField.setPosition(
    new Point(this.left() + this.edge, this.top() + y + this.edge)
  );
  this.inputField.setWidth(this.width() - this.edge * 2 - this.border);
  this.button.setCenter(this.inputField.center());
  this.button.setRight(this.inputField.right() - this.border);
  this.bounds.setHeight(this.inputField.bottom() - this.top() + this.edge);
};
...
StagePickerMorph.prototype.dataRepresentation = function (data) {
  ...
  if (data instanceof SpeechBubbleMorph) {
    ...
    data.color = data.isPointingRight
...
StagePickerMorph.prototype.popup = function (stage, pos) {
  ...
  if (this.items.length < 1 && !this.title) {
...
StagePickerMorph.prototype.createItems = function (scale) {
  ...
  if (this.label) {
    ...
    this.label.text.setPosition(
      this.label.center().subtract(this.label.text.extent().floorDivideBy(2))
    );
  }
};
...
StagePickerItemMorph.prototype.popUpSubmenu = function () {
  ...
  if (this.action.bottom() > stage.bottom()) {
    ...
    scroller.setHeight(stage.bottom() - scroller.top() - this.action.edge - 2);
    scroller.adjustScrollBars();
  }
