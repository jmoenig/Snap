...
BooleanSlotMorph, XML_Serializer, SnapTranslator, SnapActions*/
...
BlockEditorMorph.prototype.popUp = function (silent) {
    ...
    if (world) {
        ...
        // FIXME: Can I use keyboard focus instead?
...
*/
    ...
    new InputSlotDialogMorph(
        ...
        () => {
            ...
            if (frag.isDeleted) {
                SnapActions.deleteBlockLabel(this.parent.definition, this);
            } else if (frag.labelString) {
                SnapActions.updateBlockLabel(this.parent.definition, this, frag);
            }
        },
        this,
...
BlockExportDialogMorph.prototype.exportBlocks = function (callback) {
    var str = this.serializer.serialize(this.blocks, true),
...
BlockRemovalDialogMorph.prototype.removeBlocks = async function () {
    var ide = this.target.parentThatIsA(IDE_Morph);

    if (!ide) {return; }
    if (this.blocks.length > 0) {
        await SnapActions.deleteCustomBlocks(this.blocks);
        ide.flushPaletteCache();
        ide.refreshPalette();
        ide.showMessage(
