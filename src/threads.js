...
ThreadManager.prototype.toggleSingleStepping = function () {
  Process.prototype.enableSingleStepping = !Process.prototype
...
function Process(topBlock, receiver, onComplete, yieldFirst) {
  ...
  if (topBlock) {
    this.homeContext.variables.parentFrame = this.homeContext.receiver.variables
    this.context = new Context(null, topBlock.blockSequence(), this.homeContext)
    if (yieldFirst) {
...
Process.prototype.evaluate = function (context, args, isCommand) {
  ...
  if (context instanceof Function) {
    ...
    // }
    return context.apply(this.blockReceiver(), args.itemsArray().concat([this]))
  }
...
JSCompiler.prototype.compileFunction = function (aContext, implicitParamCount) {
  ...
  if (block instanceof CommandBlockMorph) {
    return Function.apply(null, parms.concat([this.compileSequence(block)]))
  }
...
JSCompiler.prototype.compileExpression = function (block) {
  ...
  switch (selector) {
    ...
    case 'doSetVar': // redirect var to process
      return (
        'arguments[arguments.length - 1].setVarNamed(' +
        this.compileInput(inputs[0]) +
        ',' +
        this.compileInput(inputs[1]) +
        ')'
    ...
    case 'doChangeVar': // redirect var to process
      return (
        'arguments[arguments.length - 1].incrementVarNamed(' +
        this.compileInput(inputs[0]) +
        ',' +
        this.compileInput(inputs[1]) +
        ')'
    ...
    default:
      ...
      if (isSnapObject(target)) {
        ...
        return (
          ...
          selector +
          '.apply(arguments[arguments.length - 1], [' +
          args +
          '])'
        )
      }
...
JSCompiler.prototype.compileInput = function (inp) {
  ...
  if (inp.isEmptySlot && inp.isEmptySlot()) {
    ...
    if (inp.selector === 'reportGetVar') {
      ...
      return (
        'arguments[arguments.length - 1].getVarNamed("' + inp.blockSpec + '")'
      )
    }
