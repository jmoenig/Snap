...
Rectangle.prototype.center = function () {
  return this.origin.add(this.corner.subtract(this.origin).divideBy(2));
};
...
Morph.prototype.setCenter = function (aPoint) {
  this.setPosition(aPoint.subtract(this.extent().divideBy(2)));
};

Morph.prototype.setFullCenter = function (aPoint) {
  this.setPosition(aPoint.subtract(this.fullBounds().extent().divideBy(2)));
};
...
Morph.prototype.getPixelColor = function (aPoint) {
  ...
  context = this.getImage().getContext("2d", { willReadFrequently: true });
...
CursorMorph.prototype.processInput = function (event) {
  ...
  if (target.isNumeric) {
    filteredContent = CursorMorph.prototype.filterText(textarea.value);
  } else {
...
SliderMorph.prototype.mouseMove = function (pos) {
  ...
  if (this.orientation === "vertical") {
    ...
    newY = Math.max(
      Math.min(pos.y - this.offset.y, this.bottom() - this.button.height()),
      this.top()
    );
  } else {
    newY = this.button.bounds.origin.y;
    newX = Math.max(
      Math.min(pos.x - this.offset.x, this.right() - this.button.width()),
      this.left()
    );
  }
...
InspectorMorph.prototype.init = function (target) {
  ...
  this.isDraggable = true;
  this.border = 1;
  this.edge = MorphicPreferences.isFlat ? 5 : 5;
...
MenuMorph.prototype.init = function (
  target,
  title,
  environment,
  fontSize,
...
MenuMorph.prototype.addLine = function (width, color) {
  this.items.push([0, width || 1, undefined, color || undefined]);
};
  ...
  this.children.forEach((item) => {
    ...
    if (item === this.label) {
      item.text.setPosition(
        item.center().subtract(item.text.extent().divideBy(2))
      );
    }
  });
};
...
TriggerMorph.prototype.init = function (
  ...
  this.fontStyle = fontStyle || "sans-serif";
  this.highlightColor = isNil(bakColor)
    ? new Color(192, 192, 192)
    : bakColor.darker();
  this.pressColor = isNil(bakColor)
...
TriggerMorph.prototype.fixLayout = function () {
  this.label.setPosition(
    this.center().subtract(this.label.extent().divideBy(2))
  );
};
...
MenuItemMorph.prototype.delaySubmenu = function () {
  this.schedule = this.world().schedule(() => this.popUpSubmenu(), 500);
};
...
HandMorph.prototype.processMouseDown = function (event) {
  ...
  if (this.children.length !== 0) {
    ...
    if (this.world.cursor) {
      if (
        morph !== this.world.cursor.target &&
...
HandMorph.prototype.processTouchMove = function (event) {
  ...
  MorphicPreferences.isTouchDevice = true;
  if (
    this.touchStartPosition.distanceTo(pos) < MorphicPreferences.grabThreshold
  ) {
    return;
  }
  if (event.touches.length === 1) {
...
HandMorph.prototype.processMouseMove = function (event) {
  ...
  mouseOverNew = this.morphAtPointer()
    .allParents()
    .concat(this.inputTarget ? this.inputTarget.allParents() : []);
...
WorldMorph.prototype.getGlobalPixelColor = function (point) {
  ...
  var dta = Morph.prototype.fullImage
    .call(this)
    .getContext("2d")
...
WorldMorph.prototype.hasUnsavedEdits = function () {
  ...
  return this.children.some(
    (any) => any.hasUnsavedEdits && any.hasUnsavedEdits()
  );
};
