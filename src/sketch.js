...
VectorShape.prototype.copy = function (newShape) {
  var shape =
    newShape ||
    new VectorShape(this.borderWidth, this.borderColor, this.fillColor);
...
VectorRectangle.fromSVG = function (svg) {
  ...
  return new VectorRectangle(
    ...
    attributes.stroke === "none" ? null : Color.fromString(attributes.stroke), // borderColor
    attributes.fill === "none" ? null : Color.fromString(attributes.fill), // fillColor
    new Point(parseInt(attributes.x), parseInt(attributes.y)), // origin
...
VectorLine.prototype.containsPoint = function (aPoint) {
  var lineLength = this.origin.distanceTo(this.destination),
    distancesSum =
      aPoint.distanceTo(this.origin) + aPoint.distanceTo(this.destination);

  return (
    Math.abs(lineLength - distancesSum) <= Math.sqrt(this.borderWidth / 2) / 2
  );
};
...
VectorEllipse.fromSVG = function (svg) {
  ...
  return new VectorEllipse(
    ...
    new Point(
      ...
      parseInt(attributes.cy) + parseInt(attributes.ry)
    ) // destination
  );
};
...
VectorEllipse.prototype.toString = function () {
  return (
    VectorEllipse.uber.toString.call(this) +
    " center: " +
    this.origin.toString() +
    " radii: " +
    this.hRadius().toString() +
    "," +
  ...
  var attributes = svg.attributes,
    points = attributes.d
      ...
      .map(function (pointString) {
        ...
        return new Point(parseInt(pointArray[0]), parseInt(pointArray[1]));
      });
...
VectorPolygon.prototype.containsPoint = function (aPoint) {
  ...
  if (this.isClosed) {
    for (i = 0, j = pointCount - 1; i < pointCount; i += 1) {
      if (
        this.points[i].y > aPoint.y !== this.points[j].y > aPoint.y &&
        aPoint.x <
  ...
  function pointIsBetween(a, b) {
    return (
      Math.abs(
        a.distanceTo(b) - (aPoint.distanceTo(a) + aPoint.distanceTo(b))
      ) <=
...
VectorPolygon.prototype.drawOn = function (aCanvasMorph) {
  var context,
    points = this.points.map(function (eachPoint) {
...
VectorPaintEditorMorph.prototype.buildEdits = function () {
  ...
  this.edits.add(
    this.pushButton("undo", function () {
      myself.undo();
    })
  );

  this.edits.add(
    this.pushButton("clear", function () {
      myself.paper.clearCanvas();
    })
  );

  this.edits.add(
    this.pushButton("select all", function () {
  ...
  this.edits.add(
    this.pushButton("Bitmap", function () {
...
VectorPaintEditorMorph.prototype.openIn = function (
  ...
  if (oldrc && !isEmpty) {
    ...
    this.paper.rotationCenter = this.getBounds(this.shapes)
      .origin.subtract(this.paper.bounds.origin)
      .add(oldrc);
  } else {
    ...
    switch (myself.world().currentKey) {
      ...
      case 67:
        ...
        if (myself.ctrl && myself.selection.length) {
          myself.clipboard = myself.selection.map(function (each) {
            return each.copy();
          });
        }
        break;
      ...
      case 65:
        ...
        console.warn(myself, myself.ctrl)
        break;
      case 27:
        /* Escape key */
        myself.clearSelection();
...
VectorPaintEditorMorph.prototype.buildToolbox = function () {
  var tools = {
      ...
      closedBrush: "Closed brush\n(free draw)",
      polygon: "Polygon",
      paintbucket: "Paint a shape\n(shift: edge color)",
  ...
  Object.keys(tools).forEach(function (toolName) {
    ...
    button.setPosition(new Point(left + x, top + y));
    x += button.width() + padding;
    if (toolName === "crosshairs") {
      /* this tool marks the newline */
      x = 0;
...
VectorPaintEditorMorph.prototype.populatePropertiesMenu = function () {
  ...
  pc.colorpicker = new PaintColorPickerMorph(new Point(180, 100), function (
    color,
    isSecondary
  ) {
  ...
  pc.colorpicker.mouseDownRight = function (pos) {
    if (
      pos.subtract(this.position()).x > (this.width() * 2) / 3 &&
  ...
  c.add(alignNames);
  c.add(alignColor);
  label = new StringMorph(localize("Brush size") + ":", 10, null, true);
  label.color = IDE_Morph.prototype.buttonLabelColor;
  brushControl.add(label);
...
VectorPaintEditorMorph.prototype.renderColorSelection = function (
  ...
  if (color === "transparent" || color.a === 0) {
    for (i = 0; i < 180; i += 5) {
      for (j = 0; j < 15; j += 5) {
        ctx.fillStyle =
          ((j + i) / 5) % 2 === 0 ? "rgba(0, 0, 0, 0.2)" : "rgba(0, 0, 0, 0.5)";
...
VectorPaintEditorMorph.prototype.dragSelection = function (pos, optDelta) {
  var origin, ratio, delta;
  console.warn(pos)
  if (this.lastDragPosition) {
    if (this.moving) {
      ...
      if (this.shift) {
        ...
        pos = this.originalSelection.destination
          .subtract(origin)
          .multiplyBy(ratio)
          .add(origin);
      }
      ...
      delta = pos
        .subtract(this.currentShape.origin)
        .divideBy(this.lastDragPosition.subtract(this.currentShape.origin));
...
VectorPaintEditorMorph.prototype.getSVG = function () {
  ...
  svg.children = this.shapes.map(function (shape) {
    return shape.asSVG();
  });
...
VectorPaintEditorMorph.prototype.getBounds = function (shapeCollection) {
  ...
  return shapeBounds.reduce(function (previous, current) {
    return new Rectangle(
...
VectorPaintEditorMorph.prototype.checksum = function () {
  return JSON.stringify(this.shapes)
    .split("")
    .reduce(function (previousSum, currentChar) {
...
VectorPaintCanvasMorph.prototype.clearCanvas = function () {
  ...
  this.mask
    .getContext("2d")
    .clearRect(0, 0, this.bounds.width(), this.bounds.height());
  this.redraw = true;
};
...
VectorPaintCanvasMorph.prototype.mouseMove = function (pos) {
  ...
  if (this.currentTool === "paintbucket") {
    ...
    editor.currentShape &&
    editor.currentShape.isSelection &&
    !editor.selecting
  ) {
    ...
    !editor.currentShape ||
    (editor.currentShape.isSelection && !editor.selecting)
  ) {
...
VectorPaintCanvasMorph.prototype.mouseClickLeft = function (pos) {
  ...
  this.mouseEnter();
  editor.updateHistory();
  this.parentThatIsA(VectorPaintEditorMorph).ok();
};
...
VectorPaintCanvasMorph.prototype.shapeAt = function (position) {
  var editor = this.parentThatIsA(VectorPaintEditorMorph);
  return detect(editor.shapes.slice().reverse(), function (shape) {
...
VectorPaintCanvasMorph.prototype.floodfill = function (sourcepoint) {
  ...
  if (shape) {
    shape.setColor(
      editor.shift ? this.settings.secondaryColor : this.settings.primaryColor,
      editor.shift
    );
    shape.drawOn(this);
  }
